...
export async function GET(req: NextRequest) {
  ...
  const filtrados = semCliente
    ? await db.$queryRaw<{ id: number; nome: string; descricao: string | null; cor: string; status: string; cliente_id: number | null }[]>`
        SELECT id, nome, descricao, cor, status, cliente_id FROM crm_projetos
