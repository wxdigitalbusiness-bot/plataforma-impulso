...
// Para o dropdown de relink no editar
...
type Props = {
  ...
  clienteIdAtual: number;
  // Se fornecido, renderiza dropdown pra trocar de parent (modo editar).
  ...
  return (
    <form action={action} onSubmit={handleSubmit} className="space-y-6">
      ...
      {clientesDisponiveis ? (
        <label className="block">
          ...
          <p className="mt-1 text-xs text-neutral-400">
            A empresa e o WhatsApp de alertas vêm do cliente selecionado.
            Troque aqui se quiser mover esta conta para outro cliente.
          </p>
        </label>
      ) : (
        <input type="hidden" name="clienteId" value={clienteIdAtual} />
      )}
      ...
      <Field
        ...
        required
        placeholder="Ex.: Caldeiras Santesso 01"
      />
        ...
        <fieldset className="space-y-4 rounded-xl border border-blue-200 bg-blue-50/30 p-4">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <Field
              ...
              defaultValue={conta?.metaAdAccountId ?? ""}
            ...
            <Field
              ...
              type="number"
              step="0.01"
              defaultValue={conta ? String(conta.limiteMinimo) : "100"}
              required={usaMeta}
            />
            <Field
              label="Moeda"
              name="moeda"
              defaultValue={conta?.moeda ?? "BRL"}
            />
          </div>
          <Toggle
            name="receberAlertaSaldo"
            label="Receber alerta de saldo baixo (Meta Ads)"
            defaultChecked={conta?.receberAlertaSaldo ?? true}
          />
        </fieldset>
      )}
        ...
        <fieldset className="space-y-4 rounded-xl border border-green-200 bg-green-50/30 p-4">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <Field
              ...
              defaultValue={conta?.googleAdCustomerId ?? ""}
            ...
            <Field
              ...
              type="number"
              step="0.01"
              defaultValue={conta ? String(conta.limiteMinimoGoogle) : "100"}
              required={usaGoogle}
            />
            <div>
              <Field
                ...
                name="googleAdsMccId"
                defaultValue={conta?.googleAdsMccId ?? ""}
                placeholder="8259939796"
              />
              <p className="mt-1 text-xs text-neutral-400">
                Preencha apenas se esta conta pertence a um MCC diferente do padrão (WX: 825-993-9796)
              </p>
            </div>
          </div>
          <Toggle
            ...
            defaultChecked={conta?.receberAlertaGoogle ?? false}
          />
        </fieldset>
      )}
      ...
      <Toggle
        name="ativo"
        label="Conta ativa"
        defaultChecked={conta?.ativo ?? true}
      />
